...
class DraftManager {
  ...
  async startDraft(interaction, config) {
    ...
    if (draftPlayerCount % 2 !== 0) {
      draftPlayerCount -= 1;
      adjustedDownToEven = true;
    }

    if (draftPlayerCount < config.minPlayers) {
      await interaction.reply({
        content: `Need at least ${config.minPlayers} draftable players after balancing team sizes.`,
        ephemeral: true
      });
      return;
    }
    ...
    if (adjustedDownToEven) {
      await interaction.followUp({
        content: `ℹ️ Using ${draftPlayerCount} players for balanced teams; one player will remain undrafted this round.`,
        ephemeral: false
      });
    }
  }
