...
The selfbot streams H.264 by default into a Discord voice channel via the selfbot user-token path. Arc VAAPI AV1 is available as an opt-in with `VIDEO_CODEC=av1` and `STREAMBOT_VIDEO_ENCODER=vaapi`; test client compatibility before using it for a group. This is ToS-adjacent; use a dedicated/throwaway Discord account, never a token used elsewhere, and do not run both apps on the same Discord account.
